...
func TestVerifyOutputAcceptsValidFile(t *testing.T) {
	...
	if err := VerifyOutput(context.Background(), r, "/bin/ffprobe", "/out.mp4", 45*time.Second); err != nil {
		t.Fatal(err)
	}
	want := tools.Command{
		Name: "/bin/ffprobe",
...
func TestVerifyOutputRejectsContractViolations(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := VerifyOutput(context.Background(), r, "/bin/ffprobe", "/out.mp4", 45*time.Second)
			if err == nil {
...
const validRenderProbeJSON = `{"format":{"duration":"45.000"},"streams":[{"codec_type":"video","codec_name":"h264","width":1080,"height":1920,"pix_fmt":"yuv420p","avg_frame_rate":"30/1"},{"codec_type":"audio","codec_name":"aac"}]}`
